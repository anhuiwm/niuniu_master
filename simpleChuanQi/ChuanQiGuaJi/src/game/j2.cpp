...
vector<pid_t> Paternity(vector<pid_t> &pid_vec)
{
	...
	for (size_t i=0; i<pid_vec.size(); i++) {
		...
		for (size_t j=0; j<pid_vec.size(); j++) {
...
{
	sighandler_t old = signal(signum, handler);
	if (SIG_ERR == old ) {
		//int e = errno;
...
//	char buf[40960];
	...
	//int
	size_t n = vsnprintf(p, sizeof(buf)-MAX_PATH_LEN, fmt, ap);
...
void OnClose_(SOCKET fd, MAP_SOCK_PRO &m)
{
    ...
    for( size_t i=0; i< it->second.msghdr->msg_iovlen; i++)
...
void OnClose_(SOCKET fd, int epfd, STC_SOCK_PRO &sp)
{
    ...
    for( size_t i=0; i< sp.msghdr->msg_iovlen; i++)
...
bool IPInList(DWORD dwBigEndIP, const list<STC_IP_CHECK>& lstStcIPCheck)
{
	...
	for( list<STC_IP_CHECK>::const_iterator it = lstStcIPCheck.begin(); it!= lstStcIPCheck.end(); ++it) {
		...
		if ( !it->vecStar.empty() ) {
			...
			for ( size_t i=0; i<it->vecStar.size(); i++) {
				y[ it->vecStar[i] ]=0;
			}
