...
void CMercenaryMgr::appendToclientPreviewMerStar( vector< CMercenary >::iterator& it, string& strData, CONFIG::MER_STAR_CFG* merStarCfg, CONFIG::MER_STAR_CFG* merNextStarCfg )
{
	...
	{
		insertAddAttr( i + E_ATTR_STAMINA , vecstAttrAdd, merStarCfg->baseAttrRatio[i], merNextStarCfg->baseAttrRatio[i] );
	}
...
//	  是否只有等级改变才传输 属性部分?
	...
	WORD bookIDs[COMPOSE_BOOK_NUM] = {0};
	...
	do 
	{
		...
		CONFIG::MER_STAR_CFG* merNextStarCfg = CONFIG::getMerUpstarCfg( itMer->byStar + 1 );

		COND_BREAK(merNextStarCfg == NULL, wErr, 1);
		
		DWORD  dwMaxExp = CONFIG::getMerUpstarMaxExpCfg();

		DWORD  dwNowExp = CONFIG::getMerUpstarNowExpCfg(itMer->byStar);

		DWORD wMaxAddExp = dwMaxExp - dwNowExp;
		...
		for ( auto& bookNeed : vecBookIDs )
		{
			...
			for (auto it = role->m_packmgr.lstItemPkg.begin(); it != role->m_packmgr.lstItemPkg.end() ; it++)
			{
				...
				while( oneTypeNum > 0 )
				{
					...
					if ( canGiveExp >= wMaxAddExp || haveNum >= needNum )
					{
						break;
					}
				}
		...
		while ( canGiveExp > 0 )
		{
			...
			else
			{
				...
				merStarCfg = CONFIG::getMerUpstarCfg( itMer->byStar + 1 );
				if ( merStarCfg == NULL )
				{
					break;
				}

				merNextStarCfg = merStarCfg;
			}
		}

		DWORD dwMaxStarExp = 0;

		BYTE byAttCount = 0;

		if ( merNextStarCfg != NULL )
		{
			dwMaxStarExp = merNextStarCfg->exp_need;
		}
...
//		  5:此佣兵无此光环技能
	...
	do 
	{
		...
		if ( merHaloCfg->byMoneyType == E_GOLD_MONEY)//1元宝
		{
			...
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, 0, -(int)merHaloCfg->dwMoneyCost, purpose::active_halo, 0);
			notifyIngot(role);
		}
		else if( merHaloCfg->byMoneyType == E_COIN_MONEY)//2铜钱
		{
			ADDUP(role->dwCoin, -(int)merHaloCfg->dwMoneyCost);
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, -(int)merHaloCfg->dwMoneyCost, 0, purpose::active_halo, 0);
			notifyCoin(role);
		}
		...
		for (auto it = role->m_packmgr.lstItemPkg.begin(); it != role->m_packmgr.lstItemPkg.end() ; it++)
		{
			...
			if ( haveNum >= needNum )
			{
				...
				if (merComCfg->money_type == E_GOLD_MONEY)//1元宝
				{
					...
					logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, 0, -(int)merComCfg->money_cost, purpose::synthesis_book, 0);
					notifyIngot(role);
				}
				else if(merComCfg->money_type == E_COIN_MONEY)//2铜钱
				{
					ADDUP(role->dwCoin, -(int)merComCfg->money_cost);
					logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, -(int)merComCfg->money_cost, 0, purpose::synthesis_book, 0);
					notifyCoin(role);
				}
...
int CMercenaryMgr::clientPreTrainMercenary( ROLE* role, unsigned char * data, size_t dataLen )
{
	...
	if ( trainCfg->money_type == E_GOLD_MONEY )//1元宝
	{
		...
		else
		{
			...
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, 0, -(int)trainCfg->money_cost, purpose::train_mercenary, 0);
			notifyIngot(role);
		}
	}
	else
	{
		...
		else
		{
			...
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, -(int)trainCfg->money_cost, 0, purpose::train_mercenary, 0);
			notifyCoin(role);
		}
	}
...
int CMercenaryMgr::clientRefreshSkills( ROLE* role, unsigned char * data, size_t dataLen )
{
	...
	logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, 0, -(int)needIngot, purpose::refresh_skill, 0);
	notifyIngot(role);
...
void CMercenaryMgr::insertAddAttr(const BYTE& type, vector< NETMERCENARY::TAttrExtraAdd>& vecstAttrAdd, float beforeRatio, float afterRatio )
{
	...
	for ( auto& it : attrTransCfg->vecAttrDeform )
	{
		...
		{
			vecstAttrAdd.push_back( NETMERCENARY::TAttrExtraAdd( it.attr, it.ratio * beforeRatio, it.ratio * afterRatio ) );
		}
		else
		{
			addit->addBefore += BYTE(it.ratio * beforeRatio * 100);
			addit->addAfter += BYTE(it.ratio * afterRatio * 100);
		}
	}
}
