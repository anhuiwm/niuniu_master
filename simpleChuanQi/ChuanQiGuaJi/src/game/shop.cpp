...
//  还回, [ itemid, number, quality ]n个
	...
	do 
	{
		...
		if ( byMoneyType == E_GOLD_MONEY)//1元宝
		{
			...
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, 0, -(int)dwMoneyCost, purpose::buy_item, 0);
			notifyIngot(role);
		}
		else if( byMoneyType == E_COIN_MONEY)//2铜钱
		{
			ADDUP(role->dwCoin, -(int)dwMoneyCost);
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, -(int)dwMoneyCost, 0, purpose::buy_item, 0);
			notifyCoin(role);
		}
...
//	[ shopid,discount,name,itemid,moneytype,price,number]n	
	...
	for (auto& it : m_listShopItem ) 
	{
		...
		COND_BREAK( --showNum <= 0, WORD ref, 0);//控制客户端显示数量
	}
...
//	[还回码012345]
	...
	RETURN_COND( !BASE::parseWORD( data, dataLen, shopID),  0);

	BYTE byRet = E_SHOP_SUCCESS;
	string strData;
	do 
	{
		auto it = std::find( m_listShopItem.begin(), m_listShopItem.end(), shopID );
		...
		if (sp.byMoneyType == E_GOLD_MONEY)//1元宝
		{
			...
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, 0, -(int)dwMoney, purpose::buy_item, 0);
			notifyIngot(role);
		}
		else if(sp.byMoneyType == E_COIN_MONEY)//2铜钱
		{
			ADDUP(role->dwCoin, -(int)dwMoney);
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, -(int)dwMoney, 0, purpose::buy_item, 0);
			notifyCoin(role);
		}
...
int CMargetJewellery::clientBuyAllCood( ROLE* role, unsigned char * data, size_t dataLen )
{
	...
	do 
	{
		...
		if (dwGold != 0)
		{
			...
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, 0, -(int)dwGold, purpose::buy_item, 0);
			notifyIngot(role);
		}
		...
		if (dwCoin != 0)
		{
			...
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, -(int)dwCoin, 0, purpose::buy_item, 0);
			notifyCoin(role);
		}
...
int CMargetJewellery::clientRequestUpdate( ROLE* role, unsigned char * data, size_t dataLen )
{
	...
	else
	{
		...
		logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, 0, -(int)dwCostGold, purpose::request_update, 0);
...
int CMarketMgr::clientUseMoneyTree( ROLE* role, unsigned char * data, size_t dataLen )
{
	...
	else
	{
		...
		if (vipCfg != NULL && vipCfg->money_buy > role->byUseMoneyTreeTimes)
		{
			...
			ADDUP(role->dwIngot, -(int)dwCostGold );
			ADDUP(role->dwCoin, (int)dwCoin );
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, (int)dwCoin, -(int)dwCostGold, purpose::ingot_to_coin, 0);
