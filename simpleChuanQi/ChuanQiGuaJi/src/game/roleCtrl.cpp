...
ROLE* RoleMgr::newRole( const string& serial, const string& roleName, int job, int sex, int phone_type, int platform_id )
{
	if ( _mapRoleName2Uuids.find( roleName ) != _mapRoleName2Uuids.end() )
		return NULL;
	if ( roleName.size() >= ROLE_NAME_LEN )
		return NULL;
...
ROLE* RoleMgr::loadRoleInfoFromDB(const string& uuid, DWORD dwRoleID, const string* pRoleName)
{
	char where[256]={0};
	...
	SQLSelect sqlx( DBCtrl::getSQL(), "select rolename, level, viplevel, exp, hp, money, gold, charge, mapid, cur_sceneid, fromid,unix_timestamp(lastaccess), unix_timestamp(inc_hp_time), roleblob, uuid, id,job,sex,email from "
	...
	role->faction = FactionCtrl::getFactionByRoleID( dwRoleID );
...
void RoleMgr::timerClearRoleCache( DWORD now )
{
	...
	{
		//不在线，并且下线时间超过3天
		if ( PROTOCAL::getOnlineClient(it->second->dwRoleID )== NULL && now - it->second->tLogoutTime  > OFFLINE_TIME )
		{
...
void RoleMgr::timerDailyClean( ROLE* role, time_t now ,bool bForce )
{
	...
	if ( !BASE::inTheSameDay( (time_t) role->tLastAccess, now ) )
	{
		...
		{
			//logplan("different month now=%u  last=%u", now, role->tLastAccess);
		...
		RoleMgr::resetVipByLogin(role);

		

		////每日0点发放：20011,688 402,88 403,20
		////每日11点发放：20011,588 401,20 412,3
		////每日21点发放：20011,288 706,30 401,20

		//vector<ITEM_INFO> vecItemInfo;
		//vecItemInfo.push_back( ITEM_INFO(20011, 688) );
		//vecItemInfo.push_back( ITEM_INFO(402, 88) );
		//vecItemInfo.push_back( ITEM_INFO(403, 20) );
		////char szMsg[256];
		////sprintf(szMsg,"每日0点发放：688元宝，强化石*88，套装碎片*20");

		//MailCtrl::sendMail(role->dwRoleID, E_MAIL_TYPE_PRIZE, (CONFIG:: getMsgSendOne()).c_str(), vecItemInfo);
	}
	role->tLastAccess = PROTOCAL::getCurTime();
}
...
string RoleMgr::findUuidBySerial( const string& serial, const BYTE platform_id )
{
	...
	SQLSelect sqlx( DBCtrl::getSQL(), "select uuid from "
		TABLE_ROLE_INFO " where serial = '%s' and platform_id = %u limit 1", serial.c_str(), platform_id );
...
{
	if ( role->login_day_num < VIP_NEED_LOGIN_DAY )
	{
		role->login_day_num++;

		if (role->login_day_num == 90 && role->wLevel >= 40)
		{
			role->setVipLevel(4);
		}
		else if (role->login_day_num == 30 && role->wLevel >= 40)
		{
			role->setVipLevel(3);
		}
		else if (role->login_day_num == 7 && role->wLevel >= 40)
		{
			role->setVipLevel(2);
		}
		else if ( role->login_day_num == 1 && role->wLevel >= 40 )
		{
			role->setVipLevel(1);				
		}
	}
...
void RoleMgr::triggerEleven()
{
	...
	//	//每日11点发放：20011,588 401,20 412,3
	//	//每日21点发放：20011,288 706,30 401,20
	//	//每日0点发放：688元宝，强化石*88，套装碎片*20
	//	//	每日11点发放：588元宝，金刚钻*20，3级力量宝石*3
	//	//	每日21点发放：288元宝，技能书*30，宝石精华*20

	//	vector<ITEM_INFO> vecItemInfo;
	//	vecItemInfo.push_back( ITEM_INFO(20011, 588) );
	//	vecItemInfo.push_back( ITEM_INFO(401, 20) );
	//	vecItemInfo.push_back( ITEM_INFO(412, 3) );
	//	//char szMsg[256];
	//	//sprintf(szMsg,"每日11点发放：588元宝，金刚钻*20，3级力量宝石*3");

	//	MailCtrl::sendMail(pRole->dwRoleID, E_MAIL_TYPE_PRIZE, (CONFIG:: getMsgSendTwo()).c_str(), vecItemInfo);

	//}
}

void RoleMgr::triggerTwentyone()
{
	//for (auto itr=_mapUuid2Roles.begin(); itr!=_mapUuid2Roles.end(); itr++)
	//{
	//	RolePtr pRole = itr->second;
	...
	//	vector<ITEM_INFO> vecItemInfo;
	//	vecItemInfo.push_back( ITEM_INFO(20011, 288) );
	//	vecItemInfo.push_back( ITEM_INFO(706, 30) );
	//	vecItemInfo.push_back( ITEM_INFO(401, 20) );
	//	//char szMsg[256];
	//	//sprintf(szMsg,CONFIG:: getMsgBossLastHurt()).c_str());
	//	
	//	MailCtrl::sendMail(pRole->dwRoleID, E_MAIL_TYPE_PRIZE, (CONFIG:: getMsgSendThree()).c_str(), vecItemInfo);
	//}
}
...
{
	////每天0点调用
	//TimerMgr::getMe().AddinDayStaticTimer(std::bind(&RoleMgr::triggerEleven,this), 0);
	//每天11点调用
	TimerMgr::getMe().AddinDayStaticTimer(std::bind(&RoleMgr::triggerEleven,this), 11*3600);
	//每天21点调用
	TimerMgr::getMe().AddinDayStaticTimer(std::bind(&RoleMgr::triggerTwentyone,this), 21*3600);
