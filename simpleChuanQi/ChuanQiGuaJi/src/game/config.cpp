...
namespace CONFIG
{
	...
	static bool readCfgAttributeDeform();
	static bool readCfgRechargeAward();
...
bool CONFIG::readCfgLuck(void)
{
	...
	eachIniSecLine(strIniFile, "luck", [&](  const char* line )
	{
		...
		_mapLuckCfg[ ATOW_( vecString[ 0 ] ) ] = ATOF_( vecString[ 1 ] ) / 10000.0f ;
	}
	);
	return true;
}
...
bool CONFIG::readCfgVipAdd(void)
{
	...
	{
		vector<string> vecString;
		if ( strtoken2Vec( line, SPLIT_SPACE, vecString, NULL, NULL) < 14 )	
			return;
...
//id	type	name	open_level	advance_time	begin_time	end_time	monster_id
	...
	eachIniSecLine(strIniFile, "campaign", [&](  const char* line )
	{
		...
		//activityInfoCfg.name = vecString[ 2 ] ;
		activityInfoCfg.open_level = ATOW_( vecString[ 3 ] );
		//activityInfoCfg.advance_time = vecString[ 4 ] ;
		//activityInfoCfg.begin_time = vecString[ 5 ] ;
		//activityInfoCfg.end_time = vecString[ 6 ] ;
...
void CONFIG::parseItemInfoStr( const string& str, vector<ITEM_INFO>& vecItemInfo )
{
	...
	{
		vector<string> item;
		int nums = strtoken2Vec( str.c_str(), ",", item, NULL, NULL);
...
void CONFIG::parseJobEquipInfoStr( const string& str, vector<ITEM_INFO>& vecItemInfo, size_t size )
{
	vector<string> v;
...
//star_num	stamina	strength	agility	intellect	spirit	exp_need	
	...
	eachIniSecLine(strIniFile, "mercenary_upstar", [&](  const char* line )
	{
		...
		//mercenaryStarCfg.stamina = ATOF_( vecString[ 1 ] );	
		//mercenaryStarCfg.strength = ATOF_( vecString[ 2 ] );	
		//mercenaryStarCfg.agility = ATOF_( vecString[ 3 ] );	
		//mercenaryStarCfg.intellect = ATOF_( vecString[ 4 ] );	
		//mercenaryStarCfg.spirit = ATOF_( vecString[ 5 ] );	
...
bool CONFIG::readCfgAttRandom()
{
	...
	//		WORD id = ATOW_( vecString[ 0 ] );
	//		WORD groupid = ATOW_( vecString[ 1 ] );
	//		WORD group_rate = ATOW_( vecString[ 2 ] );
	//		string parameter = vecString[ 3 ];
	//		WORD rate = ATOW_( vecString[ 4 ] );


	//		ATT_RAND_PARA para = parseAttRandPara(parameter);
	//		ATT_RAND_CFG &attRandCfg = _mapAttRandCfg[ id ];

	//		if ( attRandCfg.mapRandGroup.find(groupid) == attRandCfg.mapRandGroup.end() )
	//		{
	//			attRandCfg.vRandSumGroup.push_back( group_rate );
	//		}

	//		ATT_RAND_GROUP& randGroup = attRandCfg.mapRandGroup[ groupid ];

	//		randGroup.groupID = groupid;
	//		randGroup.groupRate = group_rate;
	//		randGroup.vRandSum.push_back(rate);
	//		randGroup.vAttRandPara.push_back(para);
	//	
	//	}
	//
	//);

	//for ( auto& it : _mapAttRandCfg )
	//{
	//	ATT_RAND_CFG & attRand = it.second;

	//	partial_sum( attRand.vRandSumGroup.begin() , attRand.vRandSumGroup.end() , attRand.vRandSumGroup.begin() );

	//	for ( auto& it : attRand.mapRandGroup )
	//	{
	//		ATT_RAND_GROUP& randGroup = it.second;			
	//		partial_sum( randGroup.vRandSum.begin() , randGroup.vRandSum.end() , randGroup.vRandSum.begin() );
	//	}
...
//void CONFIG::setRandAtt( BYTE& attr, DWORD& value, DWORD dropID , DWORD& min_value, DWORD& max_value)
//{
//	attr = 0;
//	value = 0;
//
//	auto it = _mapAttRandCfg.find( dropID );
//	if ( it==_mapAttRandCfg.end())
//		return;
//
//	const ATT_RAND_CFG &attRandCfg = it->second;
//
//
//	WORD groupID = (WORD)BASE::getIndexOfVectorSum( attRandCfg.vRandSumGroup ) + 1;
//	auto jt = attRandCfg.mapRandGroup.find(groupID);
//	if ( jt == attRandCfg.mapRandGroup.end() )
//		return;
//
//	const ATT_RAND_GROUP& randGroup = jt->second;
//	
//	int index = (WORD)BASE::getIndexOfVectorSum( randGroup.vRandSum );
//
//	const ATT_RAND_PARA& para = randGroup.vAttRandPara[index];
//
//	attr = para.attr;
//	value = (DWORD)BASE::randBetween( para.valueMin, para.valueMax+1);
//	min_value = para.valueMin;
//	max_value = para.valueMax;
//}

//void CONFIG::getRandAttRange(DWORD dropID, BYTE attrType, DWORD& min_value, DWORD& max_value)
//{
//	auto it = _mapAttRandCfg.find( dropID );
//	if ( it==_mapAttRandCfg.end())
//		return;
//
//	const ATT_RAND_CFG &attRandCfg = it->second;
//	for (auto itr=attRandCfg.mapRandGroup.begin(); itr!=attRandCfg.mapRandGroup.end(); itr++)
//	{
//		const ATT_RAND_GROUP& randGroup = itr->second;
//		for (auto itr2=randGroup.vAttRandPara.begin(); itr2!=randGroup.vAttRandPara.end(); itr2++)
//		{
//			const ATT_RAND_PARA& att = *itr2;
//			if (att.attr == attrType)
//			{
//				min_value = att.valueMin;
//				max_value = att.valueMax;
//				return;
//			}
//		}
//	}
...
bool CONFIG::readCfgOpenActivity()
{
	...
	eachIniSecLine(strIniFile, "open_activity", [&](  const char* line )
	{
		...
		RETURN_VOID(openActivityCfg.bySwitch == 0);//关闭开关
		...
		{
			openActivityCfg.reward_begin_time = ATOUL_(strBegin);
			openActivityCfg.reward_end_time = ATOUL_(strEnd);
		}
...
{
	auto it = _mapVipAddCfg.find( wLevel );

	if ( it == _mapVipAddCfg.end() )
	{
		return 0;
	}
	else
	...
	//if ( wLevel >= _mapVipAddCfg.size() )
	//	return 0;

	//return _mapVipAddCfg[ wLevel ].upgrade_exp;
}

DWORD CONFIG::addRoleExp( ROLE* role, DWORD& dwCurExp, DWORD dwAddExp, bool bKillMon  )
{
	//return addExp( wCurLevel, dwCurExp, dwAddExp, _vecExpCfg.size(), getRoleUpgradeExp );
	...
	//	if ( icLingZhu && icLingZhu->dwLingZhuExp < icLingZhu->itemCfg->hp_max )
	//	{
	//		ADDUP3( icLingZhu->dwLingZhuExp, dwAddexp * icLingZhu->itemCfg->mp_max, icLingZhu->itemCfg->hp_max);
	//	}
	//}
...
@读取配置
*/
void CONFIG::reloadLogCfg()
{
	string strIniFile = string( getProgDir() )+ STR_FSEP + "server.ini";
	//string listenIPPort =  BASE::readIni<string>( strIniFile, "server", "listen", "0.0.0.0:9999" );

	string _logIP =  BASE::readIni<string>( strIniFile, "logserver", "ip", "127.0.0.1" );
	int _logPort =  BASE::readIni<int>( strIniFile, "logserver", "port", 6400 );

	if ( _logPort )
	{
		resetLogserver(_logIP.c_str(), _logPort);
	}
}

void CONFIG::resetLogserver(const char* ip, int port)
{

	LOG_RESET_LOGSERVER* p = (LOG_RESET_LOGSERVER*) malloc( sizeof (LOG_RESET_LOGSERVER) );
	p->wLen = sizeof(LOG_RESET_LOGSERVER);
	p->wCmdType = RESET_LOGSERVER_CMD;	
	p->IP = CStr2BigendianIP( ip );
	p->port = port;

	LOGINTF->send( LogIntf::LOG_MESSAGE( p ) );
...
{	
	...
	for( size_t i=0; i<dropTimes; i++)
	{
		...
		//vector<ITEM_INFO> tryV;
		//if ( !CONFIG::getRandItem( this->dropID[E_DROP_MON], tryV ) )
		//{
		//	//留空
		//	this->_vecDropEarning.push_back( DROP_EARNING( this->exp, dwCoin, NULL ) );
		//	continue;
...
//		auto it = _mapIPInfo.find(StartIPNum);
//		if ( it != _mapIPInfo.end() )
//		{
//			int jjj=0;
...
//		}
	...
	//logff("tick:%u", BASE::getTickCount() - tick);

//static int j=1;
//while(j)
//{
//	auto* x = CONFIG::findIP( 16781612 );
//	int k=0;
//}
...
bool CONFIG::reloadCfg(const string& strCfgName)
{
	...
	//readCfgDisCount();													
	//readCfgMission();															
	//readCfgWorldBossReward();				
	//////readCfgMonster , readCfgDrop 必须先加载
	//readCfgMap();
	////readCfgMsg();
	//readCfgExtendPkg();
	//readCfgSmelt();
	//readCfgForgeRresh();//打造刷新
	//readCfgRune(); // 读取符文表
	//readCfgAttributeDeform();
	//readCfgMercenaryTrain();
	//readCfgMercenary();
	//readCfgMercenaryDefinition();
	//readCfgMercenaryHalo();
	//readCfgMercenaryUpstar();
	//readCfgMercenaryComposeBook();
	//readCfgLuck();
	////initCost();
	////JSON::initGstime();
	return true;
...
//CONFIG::WORLD_BOSS_REWARD_CFG* CONFIG::getWorldBossRewardCfg(const WORD& id)
//{
//	for (size_t i = 0; i < _vecWorldBossRewardCfg.size(); i++ )
//	{
//		if (id >= _vecWorldBossRewardCfg[i].min_boss && id <= _vecWorldBossRewardCfg[i].max_boss)
//		{
//			return &_vecWorldBossRewardCfg[i];
//		}
//	}
//	return NULL;
//}
