...
namespace ItemCtrl
{
	...
	static float strength_ratio[ 11 ] = { 0.0f,1.2f,1.1f,1.1f,0.9f,1.1f,1.0f,0.8f,0.8f,1.2f,0.8f };//身体各个部位对应强化需要的强化石系数
	//(品质)稀有度	属性比例
	//	1		   90	
	//	2		   95	
	//	3		   100	
...
bool ItemCtrl::killMapMonster( ROLE* role, WORD wMapID, BYTE byDropType, list<ITEM_CACHE*>* pLstItemNotify,  list<ITEM_CACHE*>* pLstPropsNotify, list<ITEM_CACHE*>* pLstAutoSellItem, list<ITEM_CACHE*>* pLstNoSellItem, DWORD* pvAdd )
{
	...
	float succExp = nLevel <= 10 ? 0 : ( nLevel >= 20 ? 1 : ( ( nLevel - 10 ) * 0.1f ) );
	...
	logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, dwCoin, 0, purpose::kill_monster, 0);
...
//			1:失败
	...
	if ( P != NULL )
	{
		...
		for ( auto it = lstSell.begin(); it!=lstSell.end(); ++it )
		{
			...
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, price, 0, purpose::sell_item, itemCache->itemCfg->id);
...
int  ItemCtrl::clientPreViewStrengthenInfo( ROLE* role, unsigned char * data, size_t dataLen )
{
	...
	DWORD dwStoneNums = DWORD( ceil( pow( (wStrengthLevel+1), 1.7) + pow( (wItemLevel+1)/3,(1.3+0.1*wStrengthLevel) ) ) );
...
int ItemCtrl::clientSmeltEquip( ROLE* role, unsigned char * data, size_t dataLen )
{
	...
	//带强化的装备，返还强化所消耗的70%的强化石=（2.5*当前强化等级*(当前强化等级+1)*(2*当前强化等级+1)/6+(装备等级/5*当前强化等级)）*0.7结果取整
	{
		const WORD STONE_ID = 402; // 强化石
...
int ItemCtrl::clientExchangeEquip( ROLE* role, unsigned char * data, size_t dataLen )
{
	...
	DWORD smelt_value = 0;
	DWORD patch_count = 0;
	randomSuitEquip(role->byJob, getForgeEquipLevel(role), smelt_value, patch_count);
...
int ItemCtrl::clientBuyFight( ROLE* role, unsigned char * data, size_t dataLen )
{
	...
	else
	{
		...
		if (vipCfg != NULL && vipCfg->boss_challenge > index)
		{
			...
			ADDUP3( role->byDailyBuyBossTimes, 1, BYTE_MAX );
			notifyIngot(role);
		}
		else
		{
...
*/
int ItemCtrl::makeOneTypeItem(ROLE* role, CONFIG::ITEM_CFG* itemCfg, size_t itemNum, BYTE byQuality )
{
	RETURN_COND( role==NULL || itemCfg==NULL, -1 );

	list<ITEM_CACHE*> lstItemCachePkg, lstPropsCachePkg;

	if (makeItemByType(role, itemCfg, &lstItemCachePkg, &lstPropsCachePkg, ITEM_INFO( itemCfg->id, itemNum, byQuality) ) == -1)//满
	{
		return -1;
	}
...
*/
int ItemCtrl::makeItemByType(ROLE* role, CONFIG::ITEM_CFG* itemCfg, list<ITEM_CACHE*> *pListItemCachePkg, list<ITEM_CACHE*> *pListPropsCachePkg, /*list<ITEM_CACHE*> *pListItemCacheMine,*/ const ITEM_INFO & ii)
{
	if ( IS_COIN(itemCfg) )//金币
	{
		ADDUP(role->dwCoin, ii.itemNums );
		notifyTypeValuebyRoleID(role->dwRoleID, ROLE_PRO_COIN, role->dwCoin);
		logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, ii.itemNums, 0, purpose::reward, 0);
	}
	else if ( IS_INGOT(itemCfg) )//元宝
	{
		...
		logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, 0, ii.itemNums, purpose::reward, 0);
	}
	...
	//else if ( IS_VIPCOIN( itemCfg ) )//vip元宝
	//{
	//	ADDUP(role->dwVipCoin, ii.itemNums);
	//	notifyTypeValuebyRoleID(role->dwRoleID, ROLE_PRO_VIPVAR, role->dwVipCoin);
	//}
	//else if ( IS_MINE( itemCfg ) )
	//{
	//	return role->m_packmgr.makeItem(itemCfg, ii.itemNums, pListItemCacheMine, false, (BYTE)itemCfg->mp_max);//, (ii.byUseChest << 7 | 
	//	//return ItemCtrl::makeMineItem( role, itemCfg, ii.itemNums, 0, pListItemCacheMine );
	//}
	else if ( IS_EQUIP( itemCfg ) )
	{
		return role->m_packmgr.makeItem(itemCfg, ii.itemNums, pListItemCachePkg, false, ii.byQuality );	
	}
	else
	{
		return role->m_packmgr.makeItem( itemCfg, ii.itemNums, pListPropsCachePkg, false, ii.byQuality );
	}
	return 0;
}
...
//		nextCost:下次开启新格子消耗元宝
	...
	do {
		...
		if ( pkgExtendCfg->needCoin )
		{
			...
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, -(int)pkgExtendCfg->needCoin, 0, purpose::package_extend, 0);
			notifyCoin( role );
		...
		else 	if ( pkgExtendCfg->needIngot )//只会扣1个
		{
			...
			logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, 0, -(int)pkgExtendCfg->needIngot, purpose::package_extend, 0);
			notifyIngot( role );
		}
...
int useCoinCard( ROLE* role, ITEM_CACHE* &item, BYTE byOperaType = 0, BYTE bySubOpera = 0 )
{
	...
	logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, dwAddCoin, 0, purpose::coin_card, 0);
	notifyCoin( role );
...
int useIngotCard( ROLE* role, ITEM_CACHE* &item, BYTE byOperaType = 0, BYTE bySubOpera = 0 )
{
	...
	logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, 0, dwAddIngot, purpose::ingot_card, 0);
	notifyIngot( role );
...
//	  3:没有穿戴对应装备
	...
	if (nAddCoin > 0 )
	{
		...
		logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, nAddCoin, 0, purpose::bless_oil, 0);
		notifyCoin(role);
	}
...
//	  3:已达最高等级
	...
	//		logs_db_thread::singleton().consume(service::id, role->roleName, role->dwRoleID, (int)gemCfg->decompose_coin_num, 0, purpose::use_gem, 0);
	//		notifyCoin(role);
	//	}
