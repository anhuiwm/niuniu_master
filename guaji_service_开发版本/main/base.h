...
//void safe_addup(byte& target, int8 inc)
//{
//	auto oldvalue = target;
//	target += inc;
//	if (oldvalue > target)target = (byte)-1;// 如果新值比旧值小了，则最大化
//}

static void safe_addup(byte& target, int8 inc)
{
	if (inc < 0)
	{
		if ((byte)abs(inc) > target)
		{
			target = 0;
			return;
		}
...
static void safe_addup(uint16& target, int8 inc)
{
	if (inc < 0)
	{
		if ((uint16)abs(inc) > target)
		{
			target = 0;
			return;
		}
...
//static void safe_addup(uint32& target, uint32 inc)
//{
//	auto oldvalue = target;
//	target += inc;
//	if (oldvalue > target)target = (uint32)-1;// 如果新值比旧值小了，则最大化
//}
static void safe_addup(uint32& target, int32 inc)
{
	if (inc < 0)
	{
		if ((uint)abs(inc) > target)
		{
			target = 0;
			return;
		}
...
#ifdef WIN32
static void safe_addup(DWORD& target, int32 inc)
{
	if (inc < 0)
	{
		if ((uint)abs(inc) > target)
...
#define ADDUP(orig, delta) 	safe_addup(orig, delta)
...
#define RAND_HIT( per )		( BASE::randTo( 10000u ) < (DWORD)(per*10000) )
...
#define GET_ADDUP_HELPER(orig, delta, min, max) \
	( ((long long)(orig) + (int)(delta))<(int)(min) ) ? (min) : ((long long)(orig) + (int)(delta)) > (long long)(max) ? (max) : ((orig) + (delta))
...
#define GET_ADDUP_HELPER(orig, delta, min, max) \
	(\
		{\
			...
			( x < int(min) ) ? (min) : (x > (long long)(max) ? max : x);\
		}\
	)
...
#define COND_CONTINUE(cond) \
	if ( (cond) )\
	{\
	continue;\
	}

#define RETURN_COND(cond, ret) \
	if( (cond) )\
	{\
	return ret;\
	}

#define RETURN_VOID(cond) \
	if( (cond) )\
	{\
	return ;\
	}
...
#define REDUCEUNSIGNED(count, cut)\
	do\
	{\
	if( int(count) > int(cut) )\
	count -= cut;\
		else\
		count = 0;\
	} while (0);
...
#endif
	bool parseDWORD( unsigned char*& p, size_t &leftLen, uint& out );
...
#define Sleep(millisec) BASE::sleepUninteruptable(millisec/1000, millisec%1000)
	...
	float randTo(float max);
	int randTo(int max);
	size_t randTo(size_t max);
	DWORD getTickCount();


	int randBetween(int nMin, int nMax);
	int randBetween(unsigned nMin, unsigned nMax);
	int randBetween(unsigned long nMin, unsigned long nMax);
	float randBetween(float nMin, float nMax);
	int getIndexOfVectorSum(const std::vector<DWORD> &vPartialSum);
	int randInter(int min, int max);
	bool randHit(int num, int total);
