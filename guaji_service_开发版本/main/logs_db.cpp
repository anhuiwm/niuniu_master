...
void logs_db_thread::thread_proc()
{
	...
	while(true)
	{
		...
		if(!empty())
		{
			...
			if(!db.exe(msg))
			{
				logFATAL("loginsertdb %s", db.get_error().c_str());
			}
		}
...
void logs_db_thread::consume(const string& username, int roleid, int coin, int ingot, purpose purpose, int itemid)
{
	char temp[500];
	MYSQL_ESCAPE_N(rolename, username.c_str(), min<size_t>(username.size(), ROLE_NAME_LEN), ROLE_NAME_LEN);
	sprintf(temp, "insert into log_coin_consume values(0, %u,%u,'%s',%d,%d,%d,%d,%d)",
		service::id, (DWORD)time(0), rolename, roleid, coin, ingot, (int)purpose, itemid);
