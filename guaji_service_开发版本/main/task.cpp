...
#pragma  pack(pop)
	...
	static const DWORD one_day_second = 24 * 3600;
	...
	DWORD getDayFromOpenGS();

	DWORD getDayFromTime(const time_t& give_time);
...
DWORD TASK::getDayFirstTime(const time_t& give_time)
{
	struct tm * timeinfo;
	timeinfo = localtime(&give_time);
	timeinfo->tm_hour = 0;
	timeinfo->tm_min = 0;
	timeinfo->tm_sec = 0;
...
DWORD TASK::getDayFromTime(const time_t& give_time)
{
	DWORD dwNow = time(nullptr);

	DWORD dwFirstTime = getDayFirstTime( give_time );
	DWORD dwHaveTime = 0;
	if (dwNow > dwFirstTime)
	{
		dwHaveTime = dwNow - dwFirstTime;
	}

	return ( dwHaveTime / one_day_second + 1 );//第几天登陆
}

DWORD TASK::getDayFromOpenGS()
{
	time_t open_gs_time = service::open_time;

	DWORD dwNow = time(nullptr);

	if ( open_gs_time == 0 || dwNow < (DWORD)open_gs_time )
	{
		open_gs_time = dwNow;
	}

	return TASK::getDayFromTime(open_gs_time);
...
DWORD TASK::getDayFromLogin( ROLE* role )
{
	...
	return TASK::getDayFromTime(longFirstLoginTime);
}
...
void TASK::clearTimeoutOpenActivity( ROLE* role )
{
	...
	DWORD open_gs_day = getDayFromOpenGS();//开服第几天
	...
	for ( auto it = role->mapCompleteActivityIDs.begin(); it != role->mapCompleteActivityIDs.end(); )
	{
		...
		if (openActivity->date_type == 0)
		{
			dwCondTime = loginDay;
		}
		else if ( openActivity->type == 1)
		{
			dwCondTime = open_gs_day;
		}


		if( it->second.byTaskStatus != E_TASK_COMPLETE_ALREADY_GET_PRIZE  )//时间0无限制
		{
...
void TASK::getOpenActivity( ROLE* role )
{
	...
	for (auto &it : mapOpenActivityCfg)
	{
		...
		if (activity.date_type == 0)
		{
			dwCondTime = loginDay;
		}
		else if (activity.date_type == 1)
		{
			dwCondTime = open_gs_day;
		}

		COND_CONTINUE((dwCondTime<activity.begin_time&&activity.begin_time != 0) || (dwCondTime>activity.reward_end_time&&activity.end_time != 0));
...
//	]
	...
	for ( auto it = role->mapCompleteActivityIDs.begin(); it != role->mapCompleteActivityIDs.end(); it++ )
	{
		...
		if (openActivity->type == E_ACTIVITY_LOGIN || openActivity->type == E_ACTIVITY_FIRST_CHARGE )//wm待用
		{
...
//		ALREADY_GET = 5,//已经领取
	...
	do 
	{
		...
		notifyCoin(role);
		logs_db_thread::singleton().consume(role->roleName, role->dwRoleID, -(int)stReward.coin, -(int)stReward.gold, purpose::opening_activity_reward, 0);
...
int TASK::clientGetTaskReward( ROLE* role, unsigned char * data, size_t dataLen )
{
	...
	do 
	{
		...
		else if ( taskCfg->type == E_TASK_VIP_LOGIN )
		{
			ITEM_INFO para1(0, role->getVipLevel());
			RoleMgr::getMe().judgeCompleteTypeTask(role, E_TASK_VIP_LOGIN, &para1);
		}
		else if ( taskCfg->type == E_TASK_SET_FIGHT_SKILL_NUM )
		{
			role->role_skill.judgeSetSkillTask();
		}
		else if( taskCfg->type == E_TASK_FIGHT_BOSS )
		{
			ADDUP(role->byAllBuyBossTimes, 1);//策划每次挑战Boss任务完成送一次

			ITEM_INFO para(role->wWinMaxMapID, 0);
...
void TASK::init_tencent_tasks(ROLE* role)
{
	role->map_platform_task.clear();
	CONFIG::for_each_tencent_task_cfg([&role](const std::pair<uint8, CONFIG::tencent_task>& ten_task)
	{
		role->map_platform_task.insert(make_pair(ten_task.first, MAIN_TASK_INFO(ten_task.second.step)));
	}
	);
}
bool  TASK::get_tencent_task_reward(ROLE* role, const uint8 step)
{
	RETURN_COND(role == NULL, false);
	CONFIG::tencent_task *mission_cfg = CONFIG::getTencentTaskCfg(step);
	RETURN_COND(mission_cfg == NULL, false);
	auto it = role->map_platform_task.find(step);
	RETURN_COND(it == role->map_platform_task.end(), false);
	auto& task = it->second;
	RETURN_COND(task.byTaskStatus != E_TASK_COMPLETE_CAN_GET_PRIZE, false);
	//CONFIG::ITEM_CFG* itemCfg = CONFIG::getItemCfg(mission_cfg->item.itemIndex);
	//RETURN_COND(itemCfg == nullptr, false);
	//发邮件
	if (mission_cfg->item.itemIndex != 0)//无道具奖励
	{
		vector<ITEM_INFO> vecItemInfo;
		char szMsg[256];
		sprintf(szMsg,CONFIG::get_platform_task_reward().c_str());
		vecItemInfo.push_back( mission_cfg->item );
		MailCtrl::sendMail(role->dwRoleID, E_MAIL_TYPE_PRIZE, szMsg, vecItemInfo);
	}
	it->second.byTaskStatus = E_TASK_COMPLETE_ALREADY_GET_PRIZE;
	return true;
}

bool TASK::judge_tencent_task(ROLE* role, const uint8 step)
{
	RETURN_COND(role == NULL, false);
	CONFIG::tencent_task *mission_cfg = CONFIG::getTencentTaskCfg(step);
	RETURN_COND(mission_cfg == NULL, false);
	...
	RETURN_COND(it == role->map_platform_task.end(), false);
	auto& task = it->second;
	RETURN_COND(task.byTaskStatus != E_TASK_UNCOMPLETE,false);//不是未完成的
	
	if ((step == e_download) || (step == e_up_level && role->wLevel >= mission_cfg->para) || (step == e_get_level && role->wLevel > mission_cfg->para) )
	{
...
bool TASK::php_get_platform_award(const string& phone_imei, const uint8 step)
{
	ROLE* role = RoleMgr::getMe().getRoleByImei(phone_imei);// get_role_by_imei();
	judge_tencent_task(role, step);
	return get_tencent_task_reward(role, step);
}