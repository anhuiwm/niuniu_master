...
bool BASE::parseDWORD(unsigned char*& p, size_t &leftLen, uint& out)
{
	if (leftLen < sizeof(uint))
		return false;
...
size_t BASE::randTo(size_t dwMax)
{
	size_t dwMin = 0;
	if (dwMin == dwMax)
		return dwMin;
...
#endif
	dwRand = dwRand % (dwMax - min) + min;
	return dwRand;
}


float BASE::randTo(float nMax)
{
	unsigned int dwRand;

	dwRand = rand();
	return (float)((double)dwRand / RAND_MAX) * nMax;
}
...
float BASE::randBetween(float nMin, float nMax)
{
	...
	dwRand = rand();
	return (float)((double)dwRand / RAND_MAX) * (nMax-nMin) + nMin;
}
