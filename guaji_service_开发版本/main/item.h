...
#define DEFAULT_PKG_NUMS_EQUIP		 80	//装备背包默认开放格、
#define DEFAULT_PKG_NUMS_PROP		 80		//道具背包默认开放格、
...
#define MAX_PLATFORM_TASK_NUMS		 (4)		//任务最多线数
...
#define MIN_LITTLE_MOSTER_ID		   (1000)		//小怪最大值
...
ROLE_PRO_STAR = 119, //佣兵星级
	...
	E_LUCK_DEF			 =  36,//幸运防御
	E_EXTEND_ATTR_START,
	E_DEC_DIZZY_TIMES = E_EXTEND_ATTR_START,// 减少眩晕回合
	E_HURT,// 固定伤害
	E_HURT_DOWN,// 固定免伤
	E_BOSS_HURT,// 对boss伤害提升
	E_SKILL_HURT,// 技能固定伤害
	E_SKILL_HURT_DOWN,// 技能固定减伤
	E_CONSUME_MP_DOWN,// 蓝耗降低
	E_ATTR_MAX,//最大属性限制
...
enum E_TASK_STATUS
{
	...
	E_TASK_HAVEPRIZE_UNJOIN = 3,				//3组队战用 未领奖还未参加当天战斗
};
...
public:
	...
	byte byLuckPoint = 0;		//
	byte byLuckDefPoint = 0;
	byte byHole = 0;
	array<WORD, MAX_HOLE_NUMS> wStone{};
	array<AttrValue, MAX_EQUIP_EXTRA_ATTR_NUMS> attrValueExtra{};

	DWORD dwGetStrengthStoneNum = 0; // 使用强化石数量
	unsigned exp = 0;
	byte star_level = 0;
	array<equip_extend, 3> equip_extends;
...
struct ITEM_CACHE
{
	CONFIG::ITEM_CFG * itemCfg = nullptr;
	DWORD itemNums = 0;//数量
	...
	//	};
	//};
	byte byStatus = 0; 		//道具状态信息
	byte byExtraAttrNums = 0;	//保存极品属性条目数，无需保存,
	byte byStrengthLvRes = 0;	//强化等级到50 保留
	byte byQuality = 0;		//品质
	byte byStrengthLv = 0;	//强化等级 // 符文等级
	//byte byNeedUpdateClient:1 = 0;	//
	byte byDropAndSell = 0;	//掉落并且卖出,cache, 不存数据库
	byte byLuckPoint = 0;	//额外幸运,武器,祝福油附加

	byte byLuckDefPoint = 0;//额外幸运防御,武器,祝福油附加
	DWORD dwLingZhuExp = 0;	//灵珠积攒经验
	DWORD dwFightValue = 0; // 装备战力 符文经验
	byte byHole = 0; //已经开的孔数量
	byte byPkgType = 0;
	array<WORD, MAX_HOLE_NUMS> wStone{}; //宝石镶嵌孔,宝石ID; 0:未镶嵌宝石

	array<AttrValue, MAX_EQUIP_BASE_ATTR_NUMS> attrValueBase{};			//这部分可以不保存,只cache 
	array<AttrValue, MAX_EQUIP_EXTRA_ATTR_NUMS> attrValueExtra{};			//额外属性 不保存	
	array<AttrValue, MAX_EQUIP_EXTRA_ATTR_NUMS> attrOldValueExtra{};		//初始化时候得到的属性

	DWORD dwGetStrengthStoneNum = 0;  // 使用强化石数量
	byte star_level = 0;
	unsigned exp = 0;
	vector<equip_extend> equip_extends;
	...
	bool is_halidome();// 是否神器
	void gen_halidome_attr(byte lock1 = 0, byte lock2 = 0);
	void reset_halidome_value();// 重置神器值
...
{
	unsigned id = 0;
	array<char, NAME_MAX_LEN> name {};
	byte  job = 0;
	byte  sex = 0;
	unsigned time = 0;
	meet_pk_result  result = meet_pk_result::nothing;
	unsigned coin = 0;
	int reward_pkvalue = 0;
};
...
//组队每一波战斗过程
struct BlobTeamProcess//最多一百波战斗
{
	DWORD time;
	TeamFightResult vFightResult[3];//最多三人
};


struct TeamFightRecord
{
	DWORD dwTime;		    //时间
	BYTE byRound;		    //波数
	ZERO_CONS(TeamFightRecord);
	TeamFightRecord(DWORD time, BYTE round)
	{
		this->dwTime = time;
		this->byRound = round;
	}
...
public:
	...
	array<MERCENARY_DATA, MERCENARY_NUM> arrMerData{};
	...
	byte  byNewbieGuideStep = 0;											//新手引导步骤
	DWORD dwFisrtLoginTime = 0;												//初始登录时间

	MAIN_TASK_INFO openActivityInfo[MAX_TASK_OPEN_ACTIVITY_NUMS];
	
	array<DWORD, MAX_CHARGE_TYPE> arrFirstCharge{};

	byte  byRechargeState = 0;

	DWORD close_user_valid_time = 0;										// 封号有效时间
	byte deny_chat = 0;														// 禁言
	array<byte, MAX_CODE_AWARD_NUM> arrCodeAwardTypes{};							//兑换码领取
	...
	unsigned world_boss_battle_attack_index = 0;		// 世界老怪战斗攻击序号
	byte login_day_num = 0;								//累计登陆天数
	unsigned meet_pk_protect_count = 0;						// 偶遇保护数
	unsigned guild_contribute = 0;
	uint last_guild_signin = 0;
	uint meet_pk_max_combo_count = 0;
	uint meet_pk_combo_count = 0;
	int exit_guild_time = 0;
	uint16 shrine_times = 0;//圣地膜拜次数
	WORD wLoginVip = 0;
	uint bodyguard_loot_count = 0;
	uint bodyguard_buy_loot_count = 0;
	MAIN_TASK_INFO platform_tasks[MAX_PLATFORM_TASK_NUMS];//平台任务
	BlobTeamProcess arrTeamFigthProcess[100];//组队战过程
	TeamFightRecord arrTeamFightREcord[7];//组队战个人记录
	DWORD arrTeamIDs[3];//组队战队伍
	//byte res___[ 5776 ];
