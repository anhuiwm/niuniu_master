...
void item::load_config()
{
	...
	while(sub)
	{
		...
		if (!tixml_value(attr, itemid))continue;
		...
		for (int i = 0; i < 20; i++, attr = attr->NextSibling())
		{
			if (string(attr->Value()).substr(0, 4) != "attr")
			{
				logFATAL("not attribute node. id:%d", itemid);
				cout << "not attribute node." << endl;
				exit(0);
			}
			string value;
			tixml_value(attr, value);
			if (value.empty() || value == "0")
			{
				CONFIG::ITEM_CFG::equip_extend_attr_config eattr;
				temp->equip_extend.attrs.push_back(eattr);
				continue;
			}
			...
			if(value_seg.size() != 2)
			{
				logFATAL("can't use ':' split the string. id:%d", itemid);
				cout << "can't use ':' split the string." << endl;
				exit(0);
			}
			auto attr_index = atoi(value_seg[0].c_str());
			if(attr_index >= E_ATTR_MAX)
			{
				logFATAL("max attribute index error. id:%d", itemid);
				cout << "max attribute index error" << endl;
				exit(0);
			}
			...
			{
				logFATAL("can't use ';' split the range string. id:%d", itemid);
				cout << "can't use ';' split the range string" << endl;
				exit(0);
			}
			...
			for(size_t i = 0; i < range_str.size(); i++)
			{
				...
				{
					logFATAL("can't use ',' split the range. id:%d", itemid);
					cout << "can't use ',' split the range" << endl;
					exit(0);
				}
				eattr.range.push_back(make_pair((float)atoi(range_pair[0].c_str()), (float)atoi(range_pair[1].c_str())));
			}
			temp->equip_extend.attrs.push_back(eattr);
		}
		for (int i = 0; i < 7; i++, attr = attr->NextSibling())
		{
			if (string(attr->Value()).substr(0, 4) != "attr")
			{
				logFATAL("not attribute node. id:%d", itemid);
		...
		if(attr)
		{
			...
			if (!value.empty() && value != "0")
			{
				...
				temp->equip_extend.value_attr.type = (byte)attr_index;
				vector<string> values;
				BASE::strtoken2Vec(value_seg[1].c_str(), ",", values);
				for (size_t i = 0; i < values.size(); i++)
				{
					temp->equip_extend.value_attr.values.push_back((float)atoi(values[i].c_str()));
				}
			}
		...
		if(attr)
		{
			...
			if (!value.empty() && value != "0")
			{
				temp->equip_extend.all_up_attr.type = 0;
				vector<string> values;
				BASE::strtoken2Vec(value.c_str(), ",", values);
				for (size_t i = 0; i < values.size(); i++)
				{
					auto v = atoi(values[i].c_str());
					if (v > 0)
						temp->equip_extend.all_up_attr.values.push_back(v / 100.f);
					else
						temp->equip_extend.all_up_attr.values.push_back(0);
				}
			}
		}
...
void ITEM_DATA::initFromItemCache(const ITEM_CACHE* itemCache)
{
	...
	attrValueExtra = itemCache->attrOldValueExtra;
	exp = itemCache->exp;
	star_level = itemCache->star_level;
	for(auto& item : equip_extends)
		item.type = (ATTR_TYPE)0;
	for(size_t i = 0; i < itemCache->equip_extends.size(); i++)
		equip_extends[i] = itemCache->equip_extends[i];
...
{
	CONFIG::ITEM_CFG* temp = CONFIG::getItemCfg( itemData->itemIndex);
	if (temp == NULL)
		return;
	
	itemCfg = temp;
	itemNums = itemData->itemNums;
	byStatus = itemData->byStatus;
	byStrengthLv = itemData->byStrengthLv;
	byQuality = itemData->byQuality;
	byLuckPoint = itemData->byLuckPoint;
	byLuckDefPoint = itemData->byLuckDefPoint;
	dwGetStrengthStoneNum = itemData->dwGetStrengthStoneNum;

	//if ( !IS_EQUIP(itemCfg) )
	//	return;

	byHole = itemData->byHole;
	wStone = itemData->wStone;
	attrOldValueExtra = itemData->attrValueExtra;
	exp = itemData->exp;
	star_level = itemData->star_level;

	equip_extends.clear();
	for(auto& item : itemData->equip_extends)
		if(item.type!=0)equip_extends.push_back(item);
...
{
	//创建时候随机出来的一级属性
	attrValueExtra = attrOldValueExtra;
...
DWORD ITEM_CACHE::getOutputStrengthenStoneCount()
{
	...
	{
		result += this->dwGetStrengthStoneNum;//DWORD((float)( this->dwGetStrengthStoneNum ) * 0.7);
	}
...
void ITEM_CACHE::calEquipFightValue( )
{
	...
	for (size_t i = 0; i < equip_extends.size(); i++)
	{
		fValue += float(equip_extends[i].value * attItemRatio[equip_extends[i].type]);
	}
...
bool ITEM_CACHE::is_halidome()
{
	return byQuality >= 5 && itemCfg->level_limit >= 25 && itemCfg->type == TYPE_EQUIP;
}


void ITEM_CACHE::gen_halidome_attr(byte lock1, byte lock2)
{
	if (!is_halidome())
		return;

	uint extend_count = 3;
	bool added_skill_extend = false;
	set<byte> used_type;// 避免重复
	if (!equip_extends.empty())
	{
		extend_count = equip_extends.size();

		if (lock1 > 0)used_type.insert(lock1);
		if (lock2 > 0)used_type.insert(lock2);

		for (auto iter = equip_extends.begin(); iter != equip_extends.end();)
		{
			if (iter->type != lock1 && iter->type != lock2)
			{
				iter = equip_extends.erase(iter);
			}
			else
			{
				if (iter->type >= E_EXTEND_ATTR_START)
					added_skill_extend = true;
				extend_count--;// 锁一个少一条
				iter++;
			}
		}
	...
	uint i = 0;
	while (i < extend_count)
	{
		auto index = BASE::randTo(itemCfg->equip_extend.attrs.size());
		auto& attr = itemCfg->equip_extend.attrs[index];
		if (attr.type == 0)continue;
		if (used_type.find(attr.type) != used_type.end())continue;
		if (attr.type >= E_EXTEND_ATTR_START)
		{
			if (added_skill_extend)
				continue;
			else
				added_skill_extend = true;
		}
		used_type.insert(attr.type);
		equip_extend ee;
		ee.type = (ATTR_TYPE)attr.type;
		equip_extends.push_back(ee);
		i++;
	}

	reset_halidome_value();
}


void ITEM_CACHE::reset_halidome_value()
{
	//float up = 1.0f;
	for (auto& item : equip_extends)
	{
		for (auto& cfg : itemCfg->equip_extend.attrs)
			if (cfg.type == item.type)
			{
				if (star_level < cfg.range.size())
					item.value = floor(BASE::randBetween(cfg.range[star_level].first, cfg.range[star_level].second + 1));
				else
					cout << "equip forcing index out" << endl;
				break;
			}
	}

	this->calEquipFightValue();
}