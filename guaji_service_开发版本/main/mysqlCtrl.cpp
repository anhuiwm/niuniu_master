...
int mysqlRealQuery(MYSQL *mysql, const char *q, unsigned long length)
{
	...
	if ( ret != 0 )
	{
		...
		cout << "err no:" << err << " err str:" << mysql_error(mysql) << endl;
...
//		mysql_autocommit(mysql, TRUE);
		...
		cout << "mysql connected." << dbip << "/" << dbname << endl;
...
//		logFATAL("%s", m_szSqlQuery);
	...
	if (MYSQL_REAL_QUERY_SUCC == mysqlRealQuery(conn, m_szSqlQuery, len) ) 
	{
		...
		if(m_result == NULL )
			logFATAL("mysql_get_result error (%d) :%s, sql:%s", mysql_errno(conn), mysql_error(conn), m_szSqlQuery);//CR_OUT_OF_MEMORY 
	}
	else
	{
		logFATAL("query sql(%s) error (%d) :%s, sql:%s", m_szSqlQuery, mysql_errno(conn), mysql_error(conn), m_szSqlQuery);
	}
}
