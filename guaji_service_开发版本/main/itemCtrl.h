...
#define  IS_LITTLE_MOSTER( monid )  ( (monid) <= MIN_LITTLE_MOSTER_ID )
...
#define notifyTypeValuebyRoleID(dwRoleID, type, value ) \
	do {\
		session* client = service::get_online_session(dwRoleID);\
		if ( client )\
...
namespace ItemCtrl
{
	...
	bool killMapMonster(ROLE* role, WORD wMapID, BYTE byDropType, list<ITEM_CACHE*>* pLstItemNotify, list<ITEM_CACHE*>* pLstPropsNotify, list<ITEM_CACHE*>* pLstAutoSellItem, list<ITEM_CACHE*>* pLstNoSellItem, uint* pvAdd = NULL);

	void updateNotifyRoleAttr(ROLE* role, bool withMaxExp=false);

	int makeOneTypeItem(ROLE* role, CONFIG::ITEM_CFG* itemCfg, size_t itemNum, BYTE byQuality = 1, const bool checkAutoSell = false);
	int makeAllTypeItem(ROLE* role, const vector<ITEM_INFO> &vecItemInfo);
	int makeItemByType(ROLE* role, CONFIG::ITEM_CFG* itemCfg, list<ITEM_CACHE*> *pListItemCachePkg, list<ITEM_CACHE*> *pListPropsCachePkg, const ITEM_INFO & ii, const bool checkAutoSell = false);
