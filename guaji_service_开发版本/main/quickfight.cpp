...
int QUICKFIGHT::clientBuyQuickFight( ROLE* role, unsigned char * data, size_t dataLen )
{
	...
	else
	{
		...
		if (vipCfg != NULL && vipCfg->time_buy > role->byBuyQuickFightTimes)
		{
			...
			notifyIngot(role);
			logs_db_thread::singleton().consume(role->roleName, role->dwRoleID, 0, -(int)dwCostGold, purpose::buy_quick_fight, 0);
		}
		else
		{
...
BYTE QUICKFIGHT::offlineEarning( ROLE* role, DWORD fightTime, list<ITEM_CACHE*>* pLstNotifyItem,  list<ITEM_CACHE*>* pLstNotifyProps,QUICK_REPORT& stcReport )
{
	...
	DWORD dropTimes = DWORD( 0.9f * fightTime / dwCoolTime );
	...
	if (uint(time(nullptr)) < end_time)dwCoinAdd *= 2;
	//还有直接卖掉,得到铜钱
	ADDUP( role->dwCoin, dwCoinAdd );
	logs_db_thread::singleton().consume(role->roleName, role->dwRoleID, dwCoinAdd, 0, purpose::offline_earning, 0);
...
//perCost:购买价格
	...
	if(!BASE::parseWORD(data, dataLen, mapid))return 0;
	...
	unsigned roleid = 0;
	string name;
	service::get_elitemap_first_killer(mapid, roleid, name);

	S_APPEND_DWORD(strData, roleid);
	S_APPEND_BSTR(strData, name);
...
//nextCost:下次购买价格
	...
	else
	{
		...
		{
			ADDUP(role->dwIngot, -(int)dwCostGold );
			ADDUP(role->byAllBuyHeroTimes, 1);
			ADDUP(role->byDailyBuyHeroTimes, 1);
			byRemain = vipCfg->elite_time - role->byDailyBuyHeroTimes;
			notifyIngot(role);
			logs_db_thread::singleton().consume(role->roleName, role->dwRoleID, 0, -(int)dwCostGold, purpose::buy_elite_time, 0);
		}
		else
		{
			wRet  = 1 ;//vip等级不足
...
int QUICKFIGHT::clientQuickFightBoss(ROLE* role, unsigned char * data, size_t dataLen)
{
	//	//扫荡
	...
	//		  5:道具背包满
	WORD wMapID;
	RETURN_COND(!BASE::parseWORD(data, dataLen, wMapID), 0);
	WORD wRet = 0;
	string strData;
	S_APPEND_WORD(strData, 0);
	...
	else
	{
		...
		else
		{
			...
			else if (wMapID > role->wWinMaxMapID)
			{
				wRet = 3;
			}
			else
			{
				...
				ItemCtrl::killMapMonster(role, wMapID, E_DROP_BOSS, &lstEquipNotify, &lstPropsNotify, &lstAutoSellItem, &lstNoSellItem, vAdd);
				...
				for (auto it : lstNoSellItem)
				{
					...
					S_APPEND_WORD(strData, it->itemCfg->id);
					S_APPEND_WORD(strData, it->byQuality);
					S_APPEND_DWORD(strData, it->itemNums);
					CC_SAFE_DELETE(it);
				}
				...
				RoleMgr::getMe().judgeCompleteTypeTask(role, E_TASK_FIGHT_BOSS);
			}
	...
	*(WORD *)&strData[0] = wRet;
	PROTOCAL::sendClient(role->client, PROTOCAL::cmdPacket(S_QUICK_FIGHT_BOSS, strData));
	return 0;
}