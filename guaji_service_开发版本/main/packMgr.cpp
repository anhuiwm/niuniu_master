	...
	if (!IS_MERGE_ITEM(itemCfg))//不可合并的
	{
		for (size_t i = 0; i < itemNum; i++)
		{
			...
			if (checkAutoSell && role->m_packmgr.isAutoSellItem(itemCache))
			{
				...
				logs_db_thread::singleton().consume(role->roleName, role->dwRoleID, itemCfg->price, 0, purpose::auto_sell, 0);
...
{
	return clientSkillBookUse(itemid, count) == 0;
}


//#技能书道具使用
DWORD PackMgr::clientSkillBookUse(const WORD& itemID, const DWORD& itemNum)
{

	DWORD needNum = itemNum;
	...
	if (needNum > 0)
	{
		needNum = (DWORD)ItemCtrl::rmIDItemNotify(owner, owner->m_packmgr.lstItemPkg, PKG_TYPE_ITEM, itemID, needNum);
	}
