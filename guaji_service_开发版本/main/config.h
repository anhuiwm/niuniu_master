...
#define ONCE_ROLL_BASE		(10000u)		//单roll基数
...
//		this->byChestRandNums = byChest & 127;		//低7位
	...
	bool operator==(const WORD item)const
	{
...
struct SHOP_ITEM
{
	...
	bool operator == (const union_id& id)const
	{
		return (this->wID == id.shopID && this->byDiscount == id.byDiscount );
	}
...
//编号	名称	美术资源	类型	子类型	说明	堆叠数量	职业限制	性别限制	等级限制	稀有度	出售基础价格	属性随机	随机次数	属性随机出现概率	战斗力	耐力	力量	敏捷	智慧	精神	最大生命值	最大魔法值	攻击最大值	攻击最小值	防御最大值	防御最小值	技能攻击最大值	技能攻击最小值	技能防御最大值	技能防御最小值	命中值	闪避值	暴击值	暴击抵抗值	暴击伤害增加值	暴击伤害减免值	每回合魔法恢复值	每回合生命恢复值	幸运	攻击吸血	攻击吸蓝	装备掉率	金币掉率	幸运	强化	镶嵌孔概率	套装
	...
	struct ITEM_CFG 
	{
		...
		{
		public:
			vector<equip_extend_attr_config> attrs;
			equip_extend_star value_attr;// 值型属性
			equip_extend_star all_up_attr;// 所有属性
		}equip_extend;
...
//job_id	level	stamina	strength	agility	intellect	spirit	hp_max	mp_max	atk_max	atk_min	def_max	def_min	spellatk_max	spellatk_min	spelldef_max	spelldef_min	hit	dodge	crit	crit_res	crit_inc	crit_red	mp_rec	hp_rec	luck	att_hem	att_mana	equ_drop	money_drop	luck_def
	...
	//步骤	说明	类型	参数	奖励
	//step	description	type	parameter	reward
	struct tencent_task
	{
		uint8 step;
		uint8 type;
		uint16 para;
		ITEM_INFO item;
		ZERO_CONS(tencent_task);
	};

	//索引id	圣地等级	升级经验	每日最大膜拜次数	铜钱掉落加成
	struct guild_shrine
	{
		uint16 id;
		uint16 level;
		...
		uint16 time;
		float percent;
		ZERO_CONS(guild_shrine);
	};

	//怪物id	怪物名称	怪物战力	掉落金币	掉落经验	掉落物品	掉落物品概率
	struct  TD_MONSTER_CFG
	{
		WORD    id = 0;
		string  name;
		DWORD   power = 0;
	...
	CONFIG::tencent_task* getTencentTaskCfg( uint8 step );
	CONFIG::guild_shrine* get_guild_shrine_cfg(uint16 step);
	...
	CONFIG::SEND_7DAY_AWARD_CFG* getSend7DayAwardCfg(DWORD id);
	...
	DWORD get_guild_shrine_upgrade_exp(uint16 level);
	uint16 get_guild_shrine_max_level();
	...
	void parseJobEquipInfoStr( const string& str, vector<ITEM_INFO>& vecItemInfo, int size = 1 );
	...
	const string& getMsgSend7DayAward();
	// 获取宝藏奖励邮件内容格式
	const string& get_treasure();
	// 获取偶遇中断连杀邮件内容格式
	const string& get_meet_breakcombo();
	const string& get_platform_task_reward();
	...
	void for_each_tencent_task_cfg( std::function<void (const std::pair<uint8, tencent_task>& ten_task)> func);
	...
	void add_gm_name(const string& role_name);
