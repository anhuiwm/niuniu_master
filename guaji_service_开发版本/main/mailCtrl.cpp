...
//全服邮件
bool MailCtrl::send_full_scale_mail( BYTE byMailType, const string& strContent, const vector<ITEM_INFO> &vecItemInfo )
{
	SQLSelect sqlx(DBCtrl::getSQL(), "select max(id) as maxid   from " TABLE_ROLE_INFO);

	MYSQL_ROW row = sqlx.getRow();
	if ( row )
	{ 
		DWORD dwMaxID = ATOL( row[0] );
		for (DWORD  i = 1; i <= dwMaxID; i++)
		{
			MailCtrl::sendMail(i, byMailType, strContent, vecItemInfo);
		}
	}
	...
	//	MailCtrl::sendMail(dwRoleID, byMailType, strContent, vecItemInfo);
	//}
...
//		content:邮件内容.BSTR.
	...
	session* client = role ? service::get_online_session(role->dwRoleID) : NULL;
