...
struct backend_session_init
{
	backend_session_init()
	{
		...
		backend_session::msg_procs[(int)backend_protocal::full_scale_mail] = &backend_session::send_full_scale_mail;
		backend_session::msg_procs[(int)backend_protocal::get_platform_reward] = &backend_session::get_platform_task_reward;
	}
...
bool backend_session::charging(input_stream& stream)
{
	...
	if (!stream.read_uint(roleid) || !stream.read_uint(ingot_count) || !stream.read_string(charge_account))//id,gold_num,account
	{
		out.write_bool(res);
	}
	else
...
bool backend_session::query_user_info(input_stream& stream)
{
	...
	else
	{
		...
		if (role != nullptr)
		{
			...
			out.write_bool(service::get_online_role(role->dwRoleID) != nullptr);
...
bool backend_session::kick_user(input_stream& stream)
{
	...
	else
	{
		...
		{
			service::close_session(role->client);
			out.write_bool(true);
		}
	}
	send(out.get_buffer(), out.length());
...
bool backend_session::broadcast(input_stream& stream)
{
	...
	{
		BroadcastCtrl::systemBroadcast(text);
		out.write_bool(true);
	}
	send(out.get_buffer(), out.length());
	return true;
...
bool backend_session::close_user(input_stream& stream)
{
	string name;
	unsigned valid_time;
	output_stream out((u_short)backend_protocal::close_user);
...
bool backend_session::send_full_scale_mail(input_stream& stream)
{
	string  type, content, items;
	output_stream out((u_short)backend_protocal::full_scale_mail);
	if ( !stream.read_string(type) || !stream.read_string(content) || !stream.read_string(items))
	{
		out.write_bool(false);
	}
	else
	{
		...
		bool result = MailCtrl::send_full_scale_mail( ATOB_(type), content, vecItemInfo);
		out.write_bool(result);
	}
	send(out.get_buffer(), out.length());
	return true;
}

bool backend_session::get_platform_task_reward(input_stream& stream)
{
	string  phone_imei;
	uint8 step;
	output_stream out((u_short)backend_protocal::get_platform_reward);
	if (!stream.read_string(phone_imei) || !stream.read_byte(step) )
	{
		out.write_bool(false);
	}
	else
	{
		bool result = TASK::php_get_platform_award(phone_imei, step);
		out.write_bool(result);
	}
	send(out.get_buffer(), out.length());
	return true;
