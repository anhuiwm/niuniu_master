...
void ROLE::online()
{
	...
	//byArenaResult = 0;
	first_battle = true; // 用于判断是否是上线后第一战
	send_offline_report();
	login_broad();
	battle_info.reset();
...
int ROLE::meet_fight_report(const uint target_role_id, const byte result)
{
	...
	else
	{
		...
		if(target->meet_pk_combo_count >= 5)
		{
			...
			output_stream out(S_SYSTEM_BROADCAST);
			out.write_byte(e_broadcast_type::meet_break_combo);// 终结连杀广播
...
int ROLE::monster_fight_report(byte result)
{
	...
	{
		uint16 battle_delay_time = 0; // 战斗推迟时间
...
int ROLE::elite_fight_report(byte result)
{
	...
	{
		auto it = mapElite.find(mapCfg->group_id);//本组
		if (it != mapElite.end())
		{
...
int ROLE::boss_fight_report(byte result)
{
	...
	if (wCanFightMaxMapID == battle_info.mapid)//最大地图
	{
		if (CONFIG::getMapCfg(wCanFightMaxMapID + 1) != NULL)//开地图 
		{
			wCanFightMaxMapID++;
			notifyMaxMap(this);
	...
	if ((battle_info.type != E_DROP_FIRST_BOSS) && (wLevel > 7))//策划要求<8级不要减次数
	{
...
void ROLE::equip_get_forging_info(input_stream& stream)
{
	...
	case 3:
	{
		auto consume_coin = (unsigned)(pow(ic->itemCfg->level_limit / 5.0, 2.0) * 20000.0);// 20000*(装备X等级/5)^2
		unsigned need400 = unsigned(ic->itemCfg->level_limit / 2) + 1;//int（装备X等级 / 2） + 1
...
void ROLE::equip_star_up(input_stream& stream)
{
	...
	for (int i = 0; i < 5; i++)// 依次找到指定的道具，计算消耗金币
	{
		uint temp;
	...
	if (!vecItemInfos.empty())MailCtrl::makeMailItems(this, vecItemInfos);// 批量返还宝石
...
void ROLE::equip_inherit(input_stream& stream)
{
	enum result : uint16_t
	{
		...
		insufficient_coin, // 金币不足
		insufficient_fragments,// 碎片不足
		source_equip_not_exist,// 装备不存在
		target_equip_not_exist,// 目标不存在
		not_halidome,// 不是神器不可传承
		not_self,// 不可传承给自己
		not_united_type,// 类型不统一
	};
	...
	auto consume_coin = (unsigned)pow(sic->itemCfg->level_limit / 5.0, 2.0) * 20000.0;// 20000*(装备X等级/5)^2
	if (dwCoin < consume_coin)
	{
		out.write_ushort(result::insufficient_coin);
		send(out);
	...
	unsigned need400 = unsigned(sic->itemCfg->level_limit / 2) + 1;//int（装备X等级 / 2） + 1
	if (!m_packmgr.consumeItem(400, need400))
	{
		out.write_ushort(result::insufficient_fragments);
		send(out);
...
void ROLE::reward_item(vector<DROP_SEQ>& drop_items, unsigned item_id, unsigned count )
{
	...
	if (itemcfg == nullptr)
		return;
	if (-1 == ItemCtrl::makeOneTypeItem(this, itemcfg, count, 1, true) )//1自动卖出 2未卖  品质限制必须是道具
	{
...
void ROLE::battle_over(eFIGHTTYPE battle_type)
{
	...
	if (battle_type != E_FIGHT_MON)// 普通战斗不影响此数据
	{
		battle_info.reset();
	}
}
...
int gm(ROLE* role, const string& strContent)
{
	...
	else if (vecString[0] == "$openmap")//人物清空
	{
	...
	else if (vecString[0] == "$task")//清空
	{
		role->mapTask.clear();
		TASK::autoUnlockTask(role);
	}
	else if (vecString[0] == "$platform_task")//清空
	{
		role->map_platform_task.clear();
		TASK::init_tencent_tasks(role);
	}
	else if (vecString[0] == "$activity")//清空
	{
	...
	{
		if (vecString.size() < 4)//必须提供四个以上参数
		{
			return 0;
		}
	...
	else if (vecString[0] == "$test")// 测试模式开启与关闭： $test 系统名 值
	{
		if (vecString.size() >= 2)
		{
			if (vecString[1] == "worldboss")// 后接状态 0-3
			{
				if (vecString.size() >= 3)
				{
			...
			else if (vecString[1] == "treasure")// 后接时间检测开关(on/off)
			{
			...
			else if (vecString[1] == "bodyguard")// 设置镖车时间，默认清0（未开始状态），单位小时
			{
				if (vecString.size() >= 3)
			...
			else if (vecString[1] == "bodyguard_reload_config")// 重新加载配置文件
			{
				bodyguard::load();
			}
		}
	}
	else if (vecString[0] == "$dump")// 导出
	{
		if (vecString.size() >= 2)
		{
			if (vecString[1] == "treasure_rank")// 宝藏系统
			{
